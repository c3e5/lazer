...
class GCodeGenerator(object):
    ...
    def egoto(self, x, y):
        ...
        speed = 300 if self.is_on else 800
        self.emit('G%s X%f Y%f F%d' % (gtc, x, y, speed))
    ...
    def generate(self, vectors):
        ...
        self.ecomm('Auto generated file by oldschool.py')
        self.ecomm('Set units to mm')
        self.emit('G21')
        ...
        for vector in vectors:
            ...
            dst = vector[1]
            if src != pd:
                self.ecomm('Starting a new path')
                self.eoff()
...
def main():
    ...
    s = PathSimplifier()
    g = GCodeGenerator(output=open(outfilename, 'w'))
